"use client";
/**
 * Umkehrung des MatchInvoiceModal: Mario steht auf einer Rechnung und
 * sucht die passende Bank-Buchung. Default-Filter = brutto der Rechnung.
 */
...
export default function MatchTransactionModal({
  ...
  const brutto = invoice.brutto ?? 0;
  const [search, setSearch] = useState(
    brutto > 0 ? brutto.toFixed(2).replace(".", ",") : "",
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetch("/cashflow/api/buchhaltung/transactions?status=open&limit=1000", {
      cache: "no-store",
    })
      .then((r) => r.json())
      .then((j) => {
  ...
  const filtered = useMemo(() => {
    ...
    return txns
      .filter((t) => {
        // Nur Ausgaenge — Eingaenge sind keine Eingangsrechnung
        if (t.amount > 0) return false;
        const trxAbs = Math.abs(t.amount);
        const byAmount =
          Number.isFinite(qNum) && Math.abs(trxAbs - qNum) < 0.01;
        const byText =
      ...
      >
        <div className="px-5 py-3 border-b border-[color:var(--border)]">
          <div className="flex items-start justify-between">
            <div>
              <h2 className="text-lg font-semibold">Bank-Buchung zuordnen</h2>
              <div className="text-xs text-[color:var(--muted)] mt-0.5">
                Rechnung {invoice.rechnungsdatum ?? ""} ·{" "}
                <strong>{eur(invoice.brutto, invoice.waehrung ?? "EUR")}</strong>{" "}
                · {invoice.lieferant_name ?? "—"}
                {invoice.rechnung_nr && ` · RG-Nr ${invoice.rechnung_nr}`}
              </div>
            </div>
            <button
              type="button"
        ...
        <div className="px-5 py-3 border-b border-[color:var(--border)] bg-[color:var(--surface)]">
          ...
          <div className="text-xs text-[color:var(--muted)] mt-1">
            {loading
              ? "Lade Buchungen…"
              : `${filtered.length} von ${txns.length} offenen Buchungen`}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <ul className="divide-y divide-[color:var(--border)]">
            {filtered.map((t) => {
              const trxAbs = Math.abs(t.amount);
              const amountMatch = brutto > 0 && Math.abs(trxAbs - brutto) < 0.01;
              return (
                <li
                  key={t.id}
                  className={
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-[color:var(--muted)] truncate">
                        ...
                        {t.accounting_bank_accounts?.bezeichnung && (
                          <span> · {t.accounting_bank_accounts.bezeichnung}</span>
                        )}
                      </div>
                      {t.purpose && (

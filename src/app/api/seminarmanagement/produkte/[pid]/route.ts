...
async function forward(method: "PATCH" | "DELETE", pid: string, body: Record<string, unknown> | null) {
  const target = `${BOT_URL.replace(/\/+$/, "")}/api/admin/seminarmanagement/produkte/${pid}`;
...
export async function PATCH(req: NextRequest, context: RouteContext) {
  ...
  if (!/^[0-9a-f-]{36}$/i.test(pid))
    return NextResponse.json({ error: "pid invalid" }, { status: 400 });
  let body: Record<string, unknown> = {};
  try {
    body = await req.json();
  } catch {}
  return forward("PATCH", pid, body);
}
...
export async function DELETE(_req: NextRequest, context: RouteContext) {
  ...
  if (!/^[0-9a-f-]{36}$/i.test(pid))
    return NextResponse.json({ error: "pid invalid" }, { status: 400 });
  return forward("DELETE", pid, null);
}

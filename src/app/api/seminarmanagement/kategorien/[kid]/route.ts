...
async function forward(method: "PATCH" | "DELETE", kid: string, body: Record<string, unknown> | null) {
  const target = `${BOT_URL.replace(/\/+$/, "")}/api/admin/seminarmanagement/kategorien/${kid}`;
...
export async function PATCH(req: NextRequest, context: RouteContext) {
  ...
  if (!/^[0-9a-f-]{36}$/i.test(kid))
    return NextResponse.json({ error: "kid invalid" }, { status: 400 });
  let body: Record<string, unknown> = {};
  try {
    body = await req.json();
  } catch {}
  return forward("PATCH", kid, body);
}
...
export async function DELETE(_req: NextRequest, context: RouteContext) {
  ...
  if (!/^[0-9a-f-]{36}$/i.test(kid))
    return NextResponse.json({ error: "kid invalid" }, { status: 400 });
  return forward("DELETE", kid, null);
}
